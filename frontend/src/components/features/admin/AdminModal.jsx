...
export const AdminEditModal = ({ isOpen, onClose, admin = null, onSave }) => {
  ...
  useEffect(() => {
    if (isOpen && admin) {
      // Parse phone number
      const fullPhone = admin.userId?.phone || ''
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await adminAPI.updateAdmin(admin._id, {
        ...formData,
      ...
      if (response.success) {
        ...
        if (response.errors) {
          ...
          setErrors({
            general: response.message || 'An error occurred while updating the admin'
          })
        }
      }
    } catch (error) {
